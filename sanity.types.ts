...
// Query: *[_type == "listing" && defined(slug.current) &&     (      !defined($search) ||       title match $search ||       category match $search ||       author->name match $search    ) &&     (      !defined($category) ||       category match $category    )  ] | order(author->isFeatured desc, _createdAt desc) {    _id,    _type,    title,    slug,    _createdAt,    author -> {      _id,      name,      image,      email,      "isFeatured": coalesce(isFeatured, false),      "featuredSince": featuredSince,      "featuredUntil": featuredUntil    },    description,    isFeaturedListing,    category,    image,    condition,    price,    ratePeriod,    contact,    toolDetails,    deleteToken,    "ratings": *[_type == "rating" && service._ref == ^._id] {      _id,      rating,      review,      createdAt,      user-> {        _id,        name,        image      }    }  }
...
// Variable: LISTING_BY_LISTING_ID_QUERY
// Query: *[_type == "listing" && _id == $listingId][0]{  _id,  title,  description,  category,  condition,  author->{    _id,    name,    image,    email  },  slug,  _createdAt,  ratings,  image,  deleteToken,  // Make sure this is included if you need it for mutations  toolDetails,  price,  ratePeriod,  contact}
export type LISTING_BY_LISTING_ID_QUERYResult = {
  _id: string;
  title: string | null;
  description: string | null;
  category: null;
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    "*[_type == \"listing\" && _id == $listingId][0]{\n  _id,\n  title,\n  description,\n  category,\n  condition,\n  author->{\n    _id,\n    name,\n    image,\n    email\n  },\n  slug,\n  _createdAt,\n  ratings,\n  image,\n  deleteToken,  // Make sure this is included if you need it for mutations\n  toolDetails,\n  price,\n  ratePeriod,\n  contact\n}": LISTING_BY_LISTING_ID_QUERYResult;
